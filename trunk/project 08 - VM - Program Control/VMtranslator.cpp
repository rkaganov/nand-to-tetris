...
void CodeWriter::writeFunction(string functionName, int numLocals){
     ...
     functionName.erase(functionName.find("."),temp.size()+1);
...
void translate(char* filename){
     ...
     // c.writeInit(); /*Uncomment for FibonacciElements and StaticsTest*/
     while(p.hasMoreCommands()){                                
         string arg1;
         int arg2=-1;
         if(p.advance()==-1) return;
         if(p.commandType()!=C_RETURN)
         p.argument(arg1, arg2);
         switch(p.commandType()){
             case C_ARITHMETIC: if(arg2==-1) c.writeArithmetic(arg1);
                                else{
                                ...
                                break;
             case C_PUSH:
             case C_POP:        c.writePushPop(p.commandType(),arg1,arg2);
                                break;
             case C_LABEL:      c.writeLabel(arg1);
                                break;
             case C_GOTO:       c.writeGoto(arg1);
                                break;
             case C_IF:         c.writeIf(arg1);
                                break;
             case C_CALL:       c.writeCall(arg1,arg2);
                                break;
             case C_RETURN:     c.writeReturn();
                                break;
             case C_FUNCTION:   c.writeFunction(arg1,arg2);
                                break;
             default:           cout << "Invalid command Type\n";
                                getchar();
                                exit(0);
         }
     }
